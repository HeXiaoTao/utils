...
process_again:
	...
	case RF_RX_DONE:
		...
		if(_size > 0) {
			if(!IS_TOME(_raw)) {
				DBGPRINT(DEBUG_INFO, "+%s : NOT_TOME: ??????????", __FUNCTION__);
				break;
			}
...
int lora_sendmsg(struct lora_addr *addr, const struct lora_msg *msg)
{
	...
	_data->size = mac_len;
	payload = &_data->next;
	addr->node_addr = htonl(addr->node_addr);
	addr->host_addr = htons(HOST_NVRAM_ADDR);
	memcpy(payload, &addr->node_addr, NODE_ADDR_SIZE);
	payload += NODE_ADDR_SIZE;
	memcpy(payload, &addr->host_addr, HOST_ADDR_SIZE);
	payload += HOST_ADDR_SIZE;
	*payload++ = MAC_TYPE_DATA | (addr->seq & MAC_SEQ_MASK);
...
int lora_recvmsg(struct lora_addr *addr, struct lora_msg *msg)
{
	...
	if(_data != NULL) {
		...
		payload = &_data->next + HOST_ADDR_SIZE;
		memcpy(&addr->node_addr, payload, NODE_ADDR_SIZE);
		payload += NODE_ADDR_SIZE;
		addr->node_addr = ntohl(addr->node_addr);
		addr->seq = (*payload++) & MAC_SEQ_MASK;
		//resign
		_data->size = _data->size - MAC_HEAD_SIZE - MAC_CRC_SIZE;
